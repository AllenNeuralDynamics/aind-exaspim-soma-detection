...
def get_transform(opt, params=None):
    ...
    if "random3Drotate" in opt.preprocess:
        if params is None:
            transform_list += [
                transforms.Lambda(lambda img_np: __randomrotate_clean_3D_xy(img_np))
            ]
        else:
            transform_list += [
                transforms.Lambda(
                    lambda img_np: __rotate_clean_3D_xy(
                        img_np, angle=params["angle_3D"]
                    )
                )
            ]
    ...
    if "random90rotate" in opt.preprocess:
        if params is None:
            transform_list += [
                transforms.Lambda(lambda img_np: __random90rotate(img_np))
            ]
        else:
            transform_list += [
                transforms.Lambda(
                    lambda img_np: __rotate(img_np, params["rotate_params"])
                )
            ]
    ...
    if "randomflip" in opt.preprocess:
        if params is None:
            ...
            transform_list += [
                transforms.Lambda(lambda img_np: __flip(img_np, params["flip_axis"]))
            ]
    ...
    if "reorderColorChannel" in opt.preprocess:
        transform_list += [
            transforms.Lambda(lambda img_np: __reorderColorChannel(img_np))
        ]
...
def __randomcontrast(img_np, randomcontrast_val):
    ...
    if img_max == top_val:
        ...
        img_normed = (img_processed - top_val) * (
            (img_max - img_min) / (img_max - top_val)
        ) + img_min
...
def __centercrop(img_np, crop_portion):
    crop_portion = (
        100 - crop_portion * 1.0
    ) / 100  # crop_portion of 90% means cropping out 10%.

...
class SimulationCropDataSet(BaseDataset):
    """
    This class is a modified version of DiceImage dataset module, only with a custom cropping.
    ...
    def modify_commandline_options(parser, is_train=False):
        parser.add_argument(
            ...
            type=int,
            default=0,
            help="set the size of overlapping region when dicing the dataset.",
        )
        parser.add_argument(
            "--border_cut",
            default=0,
            type=int,
            help="specify how much border you want to remove in a cube-by-cube inference.",
        )
    ...
    def __init__(self, opt):
        ...
        A_img_np = io.imread(self.A_path)[z : z + 700, y : y + 700, x : x + 700]
        ...
        self.cube = DiceCube(
            A_img_np, self.roi_size, overlap=self.overlap, border_cut=self.border_cut
        )
...
class DiceCube:
    ...
    def __getitem__(self, index):
        ...
        new_cube = self.image[
            current_z - self.border_cut : current_z + self.roi_size + self.border_cut,
            current_y - self.border_cut : current_y + self.roi_size + self.border_cut,

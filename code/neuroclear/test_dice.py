...
if __name__ == "__main__":
    ...
    print(
        "Whole image step counts y,x,z: {}".format(
            (dice_assembly.y_steps, dice_assembly.x_steps, dice_assembly.z_steps)
        )
    )
    print(
        "Whole image step counts: {}".format(
            dice_assembly.y_steps * dice_assembly.x_steps * dice_assembly.z_steps
        )
    )
    ...
    if opt.save_volume:
        ...
        if opt.load_iter > 0:
            output_xy_vol_path = (
                web_dir
                + "/volumes/output_volume_xy-view_iter-"
                + str(opt.load_iter)
                + ".tif"
            )
        else:
            output_xy_vol_path = (
                web_dir
                + "/volumes/output_volume_xy-view_epoch-"
                + str(opt.epoch)
                + ".tif"
            )
    ...
    if opt.save_projections:
        ...
        util.save_image(
            fake_proj_xy,
            web_dir + "/projections/fake_xy_proj_epoch-" + str(opt.epoch) + ".tif",
        )
        util.save_image(
            fake_proj_xz,
            web_dir + "/projections/fake_xz_proj_epoch-" + str(opt.epoch) + ".tif",
        )
        util.save_image(
            fake_proj_yz,
            web_dir + "/projections/fake_yz_proj_epoch-" + str(opt.epoch) + ".tif",
        )

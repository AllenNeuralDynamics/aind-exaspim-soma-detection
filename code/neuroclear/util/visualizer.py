...
def save_test_metrics(save_dir, opt, ssims, psnrs):
    ...
    message += (
        "(ssim_avg: %.4f, psnr_avg: %.4f, ssim_whole: %.4f, psnr_whole: %.4f)\n"
        % (
            ssim_avg_input_gt,
            psnr_avg_input_gt,
            ssim_whole_input_gt,
            psnr_avg_whole_input_gt,
        )
    )
    message += "-------------------------------------------------\n"
    ...
    message += (
        ...
        % (
            ssim_avg_output_gt,
            psnr_avg_output_gt,
            ssim_whole_output_gt,
            psnr_avg_whole_output_gt,
        )
    )
    message += "-------------------------------------------------"
...
class Visualizer:
    ...
    def display_current_results(self, visuals, epoch):
        ...
        for label, image in visuals.items():
            if self.opt.model != "classifier":
                ...
                slice_portion = int(
                    d / 2
                )  # For 3D images, get three images at increasing depth
                img_sample = img_np[
                    0, 0, slice_portion, :, :
                ]  # choose the first sample in the batch
                img_sample2 = img_np[
                    0, 0, :, slice_portion, :
                ]  # choose the second sample in the batch
                img_sample3 = img_np[
                    0, 0, :, :, slice_portion
                ]  # choose the third sample in the batch
    ...
    def display_model_hyperparameters(
        self,
    ):  # note that in tensorboard, it is shown as markdowns.
        message = "--------------- Options ------------------  \n"
        ...
        for label, image in visuals.items():
            if self.display_histogram:
                self.tb_writer.add_histogram(
                    "train_histograms/" + label, image[0][0], epoch
                )
    ...
    def save_current_visuals(self, visuals, epoch):
        for label, image in visuals.items():
            ...
            file_name = os.path.join(
                self.img_dir, str(epoch) + "_" + str(label) + ".tif"
            )
        ...
        for label, loss in losses.items():
            if is_epoch:
                ...
                self.tb_writer.add_scalar(
                    "train_by_epoch_progress/" + label, loss, plot_count
                )
    ...
    def print_current_losses(self, epoch, epoch_progress, losses, t_comp, t_data):
        ...
        """
        message = (
            "(epoch: %d, epoch_progress: %d%%, iter time: %.3f, data load time: %.3f) "
            % (epoch, epoch_progress, t_comp, t_data)
        )

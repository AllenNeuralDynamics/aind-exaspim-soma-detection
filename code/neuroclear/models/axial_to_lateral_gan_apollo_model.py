...
class AxialToLateralGANApolloModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument(
                "--randomize_projection_depth",
                action="store_true",
                help="randomize the depth for MIP",
            )
            parser.add_argument(
                "--projection_depth",
                type=int,
    ...
    def __init__(self, opt):
        ...
        if not (self.randomize_projection_depth):
            ...
            print(
                "Projection depth is randomized with maximum depth of %d."
        ...
        if self.isTrain:  # define discriminators
            self.netD_A_axial = networks.define_D(
                opt.output_nc,
                opt.ndf,
                opt.netD,
                opt.n_layers_D,
            ...
            self.netD_A_lateral = networks.define_D(
                opt.output_nc,
                opt.ndf,
                opt.netD,
                opt.n_layers_D,
            ...
            self.netD_B_axial = networks.define_D(
                opt.input_nc,
                opt.ndf,
                opt.netD,
                opt.n_layers_D,
            ...
            self.netD_B_lateral = networks.define_D(
                opt.input_nc,
                opt.ndf,
                opt.netD,
                opt.n_layers_D,
        ...
        if self.isTrain:
            ...
            self.optimizer_D = torch.optim.Adam(
                itertools.chain(
                    self.netD_A_axial.parameters(),
                    self.netD_A_lateral.parameters(),
                    self.netD_B_axial.parameters(),
                    self.netD_B_lateral.parameters(),
                ),
                lr=opt.lr,
                betas=(opt.beta1, 0.999),
            )
    ...
    def set_input(self, input):
        ...
        if not (self.randomize_projection_depth):
            ...
            self.projection_depth = np.random.randint(
                max(2, self.min_projection_depth), self.max_projection_depth + 1
            )
    ...
    def backward_D_A_lateral(self):
        self.loss_D_A_lateral = self.backward_D_projection(
            self.netD_A_lateral,
            self.real,
            self.fake,
            self.lateral_axis,
            self.lateral_axis,
        )  # comparing XY_original to XY_fake_MIP

    def backward_D_A_axial(
        self,
    ):  # compares real XY slice image and fake axial MIP image.
        """Calculate GAN loss for discriminator D_A"""
        self.loss_D_A_axial_1 = self.backward_D_projection(
            self.netD_A_axial,
            self.real,
            self.fake,
            self.lateral_axis,
        ...
        self.loss_D_A_axial_2 = self.backward_D_projection(
            self.netD_A_axial,
            self.real,
            self.fake,
            self.lateral_axis,
    ...
    def backward_D_B_lateral(self):
        self.loss_D_B_lateral = self.backward_D_slice(
            self.netD_B_lateral,
            self.real,
            self.rec,
            self.lateral_axis,
    ...
    def backward_D_B_axial(self):
        """
        Calculate GAN loss for discriminator D_B, which compares the real
        axial slice image and fake axial slice image.

        """
        self.loss_D_B_axial_1 = self.backward_D_slice(
            self.netD_B_axial, self.real, self.rec, self.axial_1_axis, self.axial_1_axis
        )  # comparing YZ_original to YZ_reconstructed

        self.loss_D_B_axial_2 = self.backward_D_slice(
            self.netD_B_axial, self.real, self.rec, self.axial_2_axis, self.axial_2_axis
        )  # comparing YZ_original to YZ_reconstructed
    ...
    def backward_G(self):
        ...
        self.loss_G_A_lateral = (
            self.criterionGAN(
                self.proj_f(self.fake, self.netD_A_lateral, self.lateral_axis), True
            )
        ...
        self.loss_G_A_axial = (
            self.criterionGAN(
                self.proj_f(self.fake, self.netD_A_axial, self.axial_1_axis), True
            )
            * self.lambda_slice
            + self.criterionGAN(
                self.proj_f(self.fake, self.netD_A_axial, self.axial_2_axis), True
            )
            * self.lambda_slice
        )
        ...
        self.loss_G_B_lateral = (
            self.criterionGAN(
                self.iter_f(self.rec, self.netD_B_lateral, self.lateral_axis), True
            )
            * self.lambda_plane_target
        )
        self.loss_G_B_axial = (
            self.criterionGAN(
                self.iter_f(self.rec, self.netD_B_axial, self.axial_1_axis), True
            )
            * self.lambda_slice
            + self.criterionGAN(
                self.iter_f(self.rec, self.netD_B_axial, self.axial_2_axis), True
            )
            * self.lambda_slice
        )
    ...
    def optimize_parameters(self):
        ...
        # G_A and G_B - Ds require no gradients when optimizing Gs
    ...
    def iter_f(self, input, function, slice_axis):
        ...
        img_slice = input_tensor.get_slice(
            slice_axis
    ...
    def proj_f(self, input, function, slice_axis):
        ...
        mip = input_volume.get_projection(self.projection_depth, slice_axis, real_img=self.real)
...
class Volume:
    ...
    def get_projection(self, depth, slice_axis, real_img=None):
        # UPDATE HERE!

...
def get_norm_layer(norm_type="instance", dimension=3):
    ...
    if norm_type == "batch":
        norm_layer = functools.partial(
            batch_norm(dimension), affine=True, track_running_stats=True
        )

    elif norm_type == "instance":
        norm_layer = functools.partial(
            instance_norm(dimension), affine=False, track_running_stats=False
        )
...
def define_D(
    ...
    if netD == "basic":  # default PatchGAN classifier
        ...
        net = PixelDiscriminator(
            input_nc, ndf, norm_layer=norm_layer, dimension=dimension
        )
    elif netD == "kernelGAN":
        net = KernelPatchDiscriminator(
            input_nc, ndf, n_layers=5, norm_layer=norm_layer, dimension=dimension
        )
    else:
        raise NotImplementedError(
...
class GANLoss(nn.Module):
    ...
    def __init__(self, gan_mode, target_real_label=1.0, target_fake_label=0.0):
        ...
        )  # what's the need for a buffer here?
        self.register_buffer(
            "fake_label", torch.tensor(target_fake_label)
        )  # if you make a parameter like this, the value gets pushed to the GPU
...
class Unet_deconv(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, norm_layer=None, dimension=3):
        ...
        # Downsampling
        self.double_conv1 = double_conv(
            input_nc, start_nc, 3, 1, 1, norm_layer, dimension
        )
        self.maxpool1 = _maxpool(2)

        self.double_conv2 = double_conv(
            start_nc, start_nc * 2, 3, 1, 1, norm_layer, dimension
        )
        self.maxpool2 = _maxpool(2)

        # bottom floor
        self.bottom_layer = triple_conv(
            start_nc * 2, start_nc * 4, 3, 1, 1, norm_layer, dimension
        )

        # Upsampling = transposed convolution
        self.t_conv2 = _convtranspose(start_nc * 4, start_nc * 2, 2, 2)
        self.ex_double_conv2 = double_conv(
            start_nc * 4, start_nc * 2, 3, 1, 1, norm_layer, dimension
        )

        self.t_conv1 = _convtranspose(start_nc * 2, start_nc, 2, 2)
        self.ex_conv1_1 = last_conv(
            start_nc * 2, start_nc, 3, 1, 1, norm_layer, dimension
        )
...
class Unet_vanilla(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, norm_layer=None, dimension=3):
        ...
        self.double_conv3 = double_conv(
            start_nc * 2, start_nc * 4, 3, 1, 1, norm_layer, dimension
        )
        self.maxpool3 = _maxpool(2)

        # bottom floor
        self.bottom_layer = double_conv(
            start_nc * 4, start_nc * 8, 3, 1, 1, norm_layer, dimension
        )

        # Upsampling = transposed convolution
        self.t_conv3 = _convtrasnpose(start_nc * 8, start_nc * 4, 2, 2)
        self.ex_double_conv3 = double_conv(
            start_nc * 8, start_nc * 4, 3, 1, 1, norm_layer, dimension
        )
    ...
    def __init__(self, input_nc, output_nc, kernel_size, dimension=3):
        ...
        if dimension == 2:
            ...
            padding_size = (
                ...
                int(padding_size),
                int(padding_size),
            )  # such padding size makes sure that the image stays as the same size.
        self.convlayer = _conv(
            input_nc,
            output_nc,
...
class LinearKernel_NC(nn.Module):
    ...
    def forward(self, inputs):
        ...
        outputs_2 = self.noise_convlayer(
            inputs
        )  # TODO: maybe add some regularizer for faster learning?
...
class DeepLinearGenerator(nn.Module):
    def __init__(self, input_nc, output_nc):
        ...
        self.first_layer = nn.Conv3d(
            ...
            out_channels=input_nc * 64,
            kernel_size=narrowing_kernels[0],
            padding=3,
            bias=False,
        )
        ...
        feature_block += [
            nn.Conv3d(
                in_channels=input_nc * 64,
                out_channels=input_nc * 64,
                kernel_size=narrowing_kernels[1],
                padding=2,
                bias=False,
            )
        ]
        feature_block += [
            nn.Conv3d(
                in_channels=input_nc * 64,
                out_channels=input_nc * 64,
                kernel_size=narrowing_kernels[2],
                padding=1,
                bias=False,
            )
        ]
        ...
        for layer in range(len(unit_kernels)):
            feature_block += [
                nn.Conv3d(
                    in_channels=input_nc * int(64 * ((1 / 2) ** layer)),
                    out_channels=input_nc * int(64 * ((1 / 2) ** (layer + 1))),
                    kernel_size=unit_kernels[layer],
                    padding=0,
                    bias=False,
                )
            ]
        ...
        self.final_layer = nn.Conv3d(
            in_channels=input_nc * int(64 * ((1 / 2) ** (layer + 1))),
            out_channels=output_nc,
            kernel_size=unit_kernels[-1],
            padding=0,
            bias=False,
        )
...
class FixedLinearKernel(nn.Module):
    ...
    def forward(self, input):
        ...
        padding_size = tuple(
            padding_size.astype(int)
        )  # such padding size makes sure that the image stays as the same size.
        convd = F.conv3d(input, self.psf, stride=1, padding=padding_size)
        if (
            self.kernel_size[-1] % 2 == 0
        ):  # If kernel_size is a even number, it should be center-cropped by one.
            convd = convd[:, :, 1:, 1:, 1:]
        convd_noised = noisy("gauss", convd, sigma=self.gau_sigma, is_tensor=True)
        convd_noised = noisy(
            "poisson", convd_noised, peak=self.poisson_peak, is_tensor=True
        )
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(
        ...
        sequence += [
            _conv(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw)
        ]  # output 1 channel prediction map

        if use_sigmoid:
            print(
                "Using sigmoid in the last layer of Discriminator. Note that LSGAN may work well with this loss."
            )
            sequence += [nn.Sigmoid()]
...
class KernelPatchDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=5, norm_layer=None, dimension=3):
        ...
        self.first_layer = _conv(
            in_channels=input_nc, out_channels=ndf, kernel_size=7, bias=use_bias
        )
        ...
        self.final_layer = _conv(
            in_channels=ndf, out_channels=1, kernel_size=1, bias=use_bias
        )
